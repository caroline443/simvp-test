"""
SimVP Vanilla Baseline 训练脚本
=================================
原汁原味 SimVP：一次性预测，MSE 损失，无自回归，无分类离散化。

用法：
  python train_vanilla.py --config configs/default_vanilla.yaml
  python train_vanilla.py --config configs/default_vanilla.yaml --resume checkpoints_vanilla/checkpoint_epoch010.pth
"""
...
from models.simvp_vanilla import build_vanilla_model


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=str, default="configs/default_vanilla.yaml")
...
def train_one_epoch(model, loader, optimizer, device, cfg, epoch):
    ...
    for step, (input_frames, _target_bins, future_frames) in enumerate(loader):
        ...
        # future_frames: [B, T_out, 1, H, W], 归一化 [0,1]
...
def validate(model, loader, device, cfg):
    ...
    for input_frames, _target_bins, future_frames in loader:
        ...
        pred = model(input_frames)                   # [B, T_out, 1, H, W], [0,1]
        loss = criterion(pred, future_frames)
        if torch.isfinite(loss):
            loss_meter.update(loss.item(), input_frames.size(0))

        # 转换为 VIL 像素值
        pred_vil = (pred.squeeze(2).cpu().numpy() * vil_max)   # [B, T_out, H, W]
...
def main():
    ...
    print("[Model] 构建 SimVP Vanilla 模型...")
    model = build_vanilla_model(cfg).to(device)
    ...
    print(f"\n[Train] 开始 Vanilla Baseline 训练，共 {train_cfg['baseline_epochs']} 个 Epoch")
    print("=" * 60)
